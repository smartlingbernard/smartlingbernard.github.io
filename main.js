...
(function($) {
	$(document).ready(function(){
		$('.list-more a').click(function(){
			var flas	=	$(this).attr('href');
			var top 	= $(flas).offset().top;
			var body = $("html, body");
			body.stop().animate({scrollTop:top}, '1500');
		});		
		$('.demo-page .bot .col-lg-6').click(function(){
			$('.demo-page .bot .col-lg-6').removeClass('dud-active');
			$(this).addClass('dud-active');
		});
		$('.demo-page .bot .right').click(function(){
			$('.demo-tab-form').removeClass('show');
		});
...
"Please enter a valid phone number");
		...
		$('.demo-page .load_more').click(function(){
			...
			$('.demo-tab-form').addClass('show');
		});
		var form2 = $('#demo-vali-form2');
		form2.validate({
			rules:{
				fullname:	"required",
				email		:	{
		...
		function DisableSpecificDates(date) {		
		    ...
		    var disableddates = $( "#optiondate1" ).val();		
		    return [disableddates.indexOf(string) == -1];		
		}		
		function DisableSpecificDates2(date) {		
		    var string = jQuery.datepicker.formatDate('mm/dd/yy', date);		
		    var disableddates = $( "#optiondate2" ).val();		
		    return [disableddates.indexOf(string) == -1];		
		}		
		$( "#optiondate1" ).datepicker({ 		
			dateFormat: "mm/dd/yy",		
			minDate:'+1d', 		
		}).on('change', function() {		
	        $(this).valid();		
	    });		
		$( "#optiondate2" ).datepicker({ 		
			dateFormat: "mm/dd/yy" ,		
			minDate: '+1d'		
		}).on('change', function() {		
	        $(this).valid();		
	    });		
		function check_valid_datetime(){
			var date1 = $('#optiondate1').val();
			var date2 = $('#optiondate2').val();
			var time1 = $('#optiontime1').val();
			var time2 = $('#optiontime2').val();
			if((date1 == date2) && (time1 == time2)){
				$('#form1_submit').prop('disabled', true);
				$('.mess_valid_date').html('Please select a different time slot');
			}else{
				$('#form1_submit').removeAttr('disabled');
				$('.mess_valid_date').html('');
			}
		}
		$( "#optiondate1, #optiondate2, #optiontime1, #optiontime2" ).bind('change',function(){			
			...
			$('.select_category').change(function(){
				var cate_id	=	$(this).attr('value');
					$('.cate-tab').removeClass('show');
					$('.product-tab').removeClass('show');
					
					$('.cate-tab-'+cate_id).addClass('show');	
			});
			$('.tab-menu').change(function(){
				var menu_id	=	$(this).attr('value');
					$('.product-tab').removeClass('show');
					$('.product-tab-'+menu_id).addClass('show');
			});
	});
})(jQuery);
